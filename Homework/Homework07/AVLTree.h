...
private:
  ...
  //User Function
  void getIPL(TreeNode*&, int, double&, double&);
...
public:
  ...
  double getIPL(){
      ...
      //cout << "THIS IS YOUR PATHS: " <<paths << endl;
      return sum/paths;
}
  // Testing Functions
  bool isBalanced();
...
template <class T> void AVLTree<T>::getIPL(TreeNode *&nodePtr, int count, double &sum, double &paths){
    if (nodePtr){
