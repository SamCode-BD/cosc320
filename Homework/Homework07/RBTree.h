...
protected:
    ...
    //User Function 
    void getIPL(RBTreeNode<T>*, int, double&, double&);
...
public:
    ...
    double getIPL(){
      ...
      getIPL(root, count, sum, paths);
      //cout << "THIS IS YOUR RB SUM: " << sum << endl;
      //cout << "THIS IS YOUR RB PATHS: " <<paths << endl;
      return sum/paths;
    }   
};
...
template <class T> 
void RBTree<T>::getIPL(RBTreeNode<T> *nodePtr, int count, double &sum, double &paths){
    if (nodePtr){
        getIPL(nodePtr->left, count + 1, sum, paths);
        if (!nodePtr->left && !nodePtr->right){
            sum += count;
            paths = paths + 1;
        }
        getIPL(nodePtr->right, count + 1, sum, paths);
    }
}
